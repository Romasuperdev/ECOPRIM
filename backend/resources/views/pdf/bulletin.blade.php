...
<body>
    <h1>ECOPRIM</h1>
    <h2>Bulletin scolaire{{ $periode ? ' — '.$periode->libelle : '' }}</h2>
    ...
    <table class="infos">
        ...
        <tr><td><strong>Né(e) le :</strong></td><td>{{ $eleve->date_naissance?->format('d/m/Y') }}</td></tr>
    </table>
    ...
    <p class="moyenne-generale">
        ...
        @if($rang) — Rang : {{ $rang }}@endif
    </p>

    @if($deliberation)
        <div class="appreciation">
            <strong>Appréciation du conseil de classe :</strong>
            <p>{{ $deliberation->appreciation ?? '—' }}</p>
            @if($deliberation->decision)
                <p><strong>Décision :</strong> {{ $deliberation->decision }}</p>
            @endif
        </div>
    @endif
</body>
